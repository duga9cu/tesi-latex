...
\label{sec:internalfunct}
  ...
  \item Un \textbf{videoclip dell'ambiente di background}, aquisito con una videocamera adatta durante la registrazione dell'evento sonoro, su cui mappare i livelli \emph{SPL} con le bande di colore,
  \item Una \textbf{matrice di filtri} in grado di sintetizzare i microfoni virtuali a partire dalle capsule dell'array
  \item Un\textbf{ file \emph{xml} }contenente la mappatura in \emph{pixel} delle posizioni delle singole capsule microfoniche sulla immagine di background.
\end{itemize}
	...
	Per quanto riguarda la \textbf{registrazione multicanale} direttamente proveniente dall'array, è lo stesso host \emph{Audacity\textregistered} a farsi carico di rendere disponibile la registrazione al \emph{plug-in} tramite l'utilizzo di apposite \emph{routines}.

	La seconda componente fondamentale per la costruzione della mappa acustica è il \textbf{videoclip di background}. 
	Essendo a conoscenza del \emph{path} nel \emph{filesystem}, fornitoci in fase di configurazione dall'utente, utilizziamo alcune \emph{routines} basate sulle \emph{API} fornite dalla suite \emph{ffmpeg} per acquisire il video, salvarlo in memoria diviso in singoli frames (quindi immagini di tipo bitmap o per meglio dire \emph{wxBitmap}). 
	A tale scopo è stata creata una struttura dati complessa composta da due classi fondamentali:
	
	\begin{itemize}
	  \item la classe \emph{Video} che si occupa di modellizzare il video finale di output,
	  \item la classe \emph{VideoFrame} la quale implementa la struttura logica dei singoli frame del video e contiene tutte le informazioni di pressione, livelli \emph{SPL}, le varie bitmap di sfondo e di mappe di colore per ogni banda frequenziale, nonché le informazioni temporali di \emph{timestamp} riguardanti la sincronizzazione tra audio e relativa mappa acustica e video.
\end{itemize}
	...
	Infine il\textbf{ file \emph{xml}} viene letto attraverso le \emph{routines} della libreria \emph{tinyXml} e salvato in una struttura appositamente creata chiamata \emph{MikesCoordinates}.
	...
	Per l'implementazione di questa parallelizzazione si è utilizzata la classe fornita da \emph{wxWidgets} chiamata \emph{wxThread} in modo da mantenere lo stile di programmazione multipiattaforma prefissato. 
	Si tratta di una implementazione che aderisce fedelmente allo standard \emph{POSIX}, in modo da non stravolgere le abitudini implementative del programmatore, ma che contiene alcune classi avanzate in grado di semplificare abbastanza la gestione delle corse critiche e della mutua esclusione tra gli accessi concorrenti.
	...
	Alla fine del proprio iter procedurale andrà a salvare i suoi risultati in una struttura \emph{VideoFrame} che verrà aggiunta (nella posizione coerente con il numero di frame assegnato al thread stesso) alla struttura condivisa \emph{video}, naturalmente rispettando i principi che regolano l'accesso concorrente.	
	
	il main thread, dopo aver \emph{aspettato} tutti gli altri thread\footnote{ operazione che nello standard \emph{posix} prende il nome di \emph{join}} libererà la memoria inutilizzata e aggiungerà informazioni globali derivanti dall'analisi dell'intero video (come il massimo assoluto di fondoscala).
...
(verificare)
	...
	Il processo di filtraggio è stato affrontato in modo trasparente grazie all'utilizzo di una classe già progettata per la suite di plug-in \emph{Aurora} per \emph{Audacity\textregistered} \footnote{si veda \cite{aurora}} la quale già disponeva di tutte le funzioni membro necessarie ad implementare i filtraggi in bande d'ottava, come definiti secondo norma IEC-1260.
	...
	La struttura dati \emph{Video} conterrà le seguenti informazioni generali:
	\begin{itemize}
	...
	\end{itemize}
	Inoltre conterrà una struttura di tipo \emph{hash-map} contenenti i singoli \emph{VideoFrame} contrassegnati dal numero di frame come chiave unica.
	Ogni \emph{VideoFrame} dal canto suo, conterrà le informazioni seguenti:
	\begin{itemize}
	\item \emph{timestamp} del frame ottenuta dal progetto \emph{Audacity} al momento del salvataggio del \emph{chunck} audio,
	  \item Immagine \emph{bitmap} di background corrispondente al frame più vicino alla timestamp di cui sopra,
	...
	A questo punto, ogni \emph{videoFrame} può essere salvato su disco rigido con una funzione della libreria \emph{wxWidgets} che garantirà quindi un'operazione ad hoc funzionante su ogni piattaforma\footnote{per ottenere il video --- ???}.
	[...]\\
