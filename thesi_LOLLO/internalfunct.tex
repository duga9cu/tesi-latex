...
\end{itemize}
	...
	Il livello di fondo scala ($FS$) specificato dall'utente tramite la finestra di configurazione del \emph{ plug-in}, è inteso come livello massimo considerato dei segnali ottenuti dalla sintesi per convoluzione dei microfoni virtuali. 
	...
	\begin{enumerate}
\item determina su quale canale $n$ del progetto \emph{Audacity\textregistered} si ha il picco massimo 
...
%	\end{equation}
	...
	Il processo di filtraggio è stato affrontato in modo trasparente grazie all'utilizzo di una classe già progettata per la suite di plug-in \textsf{Aurora} per \emph{Audacity\textregistered} \footnote{si veda \cite{aurora}} la quale già disponeva di tutte le funzioni membro necessarie ad implementare i filtraggi in bande d'ottava, come definiti secondo norma IEC-1260.
	...
	\begin{itemize}
	\item \emph{timestamp} del frame ottenuta dal progetto \emph{Audacity\textregistered} al momento del salvataggio del \emph{chunck} audio,
